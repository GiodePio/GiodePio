  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(8px)' }} onClick={onClose}>
      <div style={{ background: '#15161c', border: '1px solid #2a2d38', borderRadius: 16, padding: 40, width: 380, position: 'relative' }} onClick={e => e.stopPropagation()}>
        ...
        <button onClick={() => { onSignIn('Google User'); onClose(); }} style={{ width: '100%', padding: '13px 0', background: '#fff', color: '#333', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 10 }}>
...
function Navbar({ signedIn, username, onSignIn, onSignOut }) {
  return (
...
export default function HomePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <Navbar signedIn={signedIn} username={username} onSignIn={() => setShowSignIn(true)} onSignOut={() => { setSignedIn(false); setUsername(''); }} />
      <Hero />
      <About />
      <Features />
      <Pricing />
      <Footer />
      {showSignIn && <SignInModal onClose={() => setShowSignIn(false)} onSignIn={(name) => { setSignedIn(true); setUsername(name); }} />}
    </div>
  );
}
